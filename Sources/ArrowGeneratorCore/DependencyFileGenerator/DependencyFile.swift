...
class DependencyFile {
  ...
  private func map(_ object: DependencyUiModel) -> CodeBlockItemSyntax {
    let args = [
      createTypeArgument(object.name),
  ...
  private func createResolvedCall(_ value: String?, _ id: String?) -> ExprSyntax {
    guard let value else {
      ...
      let args: LabeledExprListSyntax? = id.flatMap {
        [LabeledExprSyntax(expression: StringLiteralExprSyntax.string($0))]
      }
